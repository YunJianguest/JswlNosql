...
function del(id) {
	if (confirm('确实要删除吗?')) {
		location.href = "${contextPath}/suc/vivo/vivodraw!delete.action?fypage=${fypage}&_id="
				+ id;
	}
}
function deljp(id) {
		if (confirm('确实要删除吗?删除后不可恢复')) {
			location.href = "${contextPath}/suc/vivo/vivodraw!deletejp.action?_id="
					+ id;
		}
	}
function add(){
	 window.location.href='${contextPath}/suc/vivo/vivodraw!input.action?fypage=${fypage}';
}
...
<section>
  ...
  <div class="mainpanel">
	...
	<form  id="custinfoForm" name="custinfoForm" method="post"  action="${contextPath}/suc/vivo/vivodraw.action" >
    <div class="pageheader">
      <h2><i class="fa fa-user"></i> 微活动 <span>vivo世界杯</span></h2>
   ...
   <div class="panel-body fu10">
        <div class="row-pad-5">
            ...
            <input type="text" name="name"  value="${name}" placeholder="活动名称"  class="form-control" />
            </div> 
                ...
                <table class="table table-striped table-primary table-action mb30" >
                    <thead>
                      <tr>
                       <th class="th3 table-action">LOGO</th>
						...
						<th class="th6 table-action">奖品</th>
						<th class="th6 table-action">统计</th> 
                    ...
                    <tbody>
                      ...
                      <tr>
					    ...
					    <td title="<c:forEach items='${bean.lsps}' var='obj'>${obj.jp} 总数:${obj.total}剩余：${obj.total-obj.count}  |</c:forEach>">
					  奖品
					    </td>
					    <td>流量${bean.icount}中奖量${bean.redcount}</td>
                        <td class="table-action">
                              <div class="btn-group1">
                                  ...
                                  <ul role="menu" class="dropdown-menu pull-right"> 
                                      <li><a href="${contextPath}/suc/vivo/vivodraw!update.action?_id=${bean._id}">
                                      		<i class="fa fa-pencil "></i>&nbsp;&nbsp;&nbsp;&nbsp;修改</a></li>
                                       <li><a href="${ctx}/suc/slide.action?type=vivodraw-${bean._id}">
                                      		<i class="fa fa-plus"></i>&nbsp;&nbsp;&nbsp;&nbsp;添加幻灯片</a></li>
                                        <li><a href="javascript:del(${bean._id});"><i class="fa fa-trash-o "></i>&nbsp;&nbsp;&nbsp;&nbsp;删除</a></li>
                                       
                                        <li><a href="${contextPath}/suc/vivo/vivodraw!hdtj.action?hdid=${bean._id}">
                                      		<i class="fa fa-pie-chart"></i>&nbsp;&nbsp;&nbsp;&nbsp;中奖名单</a></li>
                                      	<li><a href="${contextPath}/suc/vivo/vivodraw!cardtj.action?hdid=${bean._id}">
                                      		<i class="fa fa-pie-chart"></i>&nbsp;&nbsp;&nbsp;&nbsp;卡包统计</a></li>
                                        <c:if test="${empty bean.toUserid}">
                                      	<li><a href="javascript:qrcode('${ctxurl}/suc/vivo/vivodraw!index.action?lucid=${bean._id}&custid=${bean.custid}&fromUserid=${fromUserid}')" ><i class="fa fa-eye"></i>&nbsp;&nbsp;&nbsp;&nbsp;查看预览</a></li>
                                        </c:if>
